...
public class Oppstart implements ActionListener {
	...
	Knapp fortsett, tilbake, fjern, sniper, sjåfør, sabotør, forfalsker;
	...
	public void velgRoller(){
		...
		fjern = new Knapp("Fjern", Knapp.HALV, new Lytter());
		innhold.add(fjern);
		innhold.add(tilbake);
	...
	public void nyfase(int f){
		...
		case VELGGJENSTANDER: velgGjenstander(); break;
		case HVEMERHVA: hvemErHva();
		informer("Følgende roller er med:");
		visRoller(spillere.visRoller(roller));break;
	...
	private void inverserKnapper(Knapp knapp){
		fjerning = !fjerning;
		for(Component k: innhold.getComponents()){
			...
			if(k.getSize().equals(Knapp.KVART)){
				if(fjerning){
					...
					if(r.id(Rolle.POLITI) || r.id(Rolle.MAFIA) || r.id(Rolle.BESTEVENN))
						k.setEnabled(true);
	...
	public void actionPerformed(ActionEvent e) {
		...
		else if(fase == VELGGJENSTANDER)
		{
			...
			if(e.getSource() == sniper)			{snipe 		= true; return; }
			else if(e.getSource() == sabotør)	{saboter 	= true; return; }
			else if(e.getSource() == sjåfør)	{flukt		= true; return; }
			else if(e.getSource() == forfalsker){forfalsk	= true; return; }

			else {
				k.spiller().setRolle(roller[indeks]);
				if(snipe) 	{ k.spiller.setMafiarolle(Mafia.SNIPER); 	snipe 	= false; }
				if(saboter) { k.spiller.setMafiarolle(Mafia.SABOTØR); 	saboter = false; }
				if(flukt) 	{ k.spiller.setMafiarolle(Mafia.FLUKT); 	flukt 	= false; }
				if(forfalsk){ k.spiller.setMafiarolle(Mafia.FORFALSKER);forfalsk= false; }
